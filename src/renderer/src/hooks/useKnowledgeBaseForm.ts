...
import {
	...
	usePreprocessProviders,
} from "@renderer/hooks/usePreprocess";
...
const createInitialKnowledgeBase = (defaultPreprocessProvider?: {
	type: "preprocess";
	...
	updated_at: Date.now(),
	version: 1,
	preprocessProvider: defaultPreprocessProvider,
});
...
export const useKnowledgeBaseForm = (base?: KnowledgeBase) => {
	...
	const { provider: defaultProvider } = useDefaultPreprocessProvider();

	// 获取默认预处理提供商
	const defaultPreprocessConfig = useMemo(() => {
		if (base?.preprocessProvider || !defaultProvider) return undefined;
		return { type: "preprocess" as const, provider: defaultProvider };
	}, [base?.preprocessProvider, defaultProvider]);

	const [newBase, setNewBase] = useState<KnowledgeBase>(
		base || createInitialKnowledgeBase(defaultPreprocessConfig),
	);
