...
export function getPreprocessProviderLogo(providerId: PreprocessProviderId) {
	switch (providerId) {
		...
		case "mineru":
			return MinerULogo;
		case "local-ocr":
			return MinerULogo; // 使用MinerU logo，因为本地OCR使用的是MinerU
		default:
			return undefined;
	}
}

type PreprocessProviderConfig = {
	websites: { official: string; apiKey: string };
};

export const PREPROCESS_PROVIDER_CONFIG: Record<
	PreprocessProviderId,
