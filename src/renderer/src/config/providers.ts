...
export const SYSTEM_PROVIDERS_CONFIG: Record<SystemProviderId, SystemProvider> =
	{
		cherryin: {
			...
			name: "本地 Qwen3 服务",
			type: "openai",
			apiKey: "sk-pyQKSv0m50NhK77R2a89832eAb914eF4B8D85882C2D06c5d",
			apiHost: "http://10.216.8.123:8015",
			models: [
				{
...
export const SYSTEM_PROVIDERS: SystemProvider[] = Object.values(
...
export const PROVIDER_URLS: Record<SystemProviderId, ProviderUrls> = {
	...
	ppio: {
		...
		websites: {
			official:
				"https://ppio.com/user/register?invited_by=JYT9GD&utm_source=github_cherry-studio&redirect=/",
			apiKey:
			...
			models:
				"https://ppio.com/model-api/product/llm-api?invited_by=JYT9GD&utm_source=github_cherry-studio",
		},
	},
	gemini: {
		api: {
	...
	doubao: {
		...
		websites: {
			...
			apiKey:
				"https://www.volcengine.com/experience/ark?utm_term=202502dsinvite&ac=DSASUQY5&rc=DB4II4FC",
	...
	qiniu: {
		...
		websites: {
			...
			apiKey:
				"https://portal.qiniu.com/ai-inference/api-key?cps_key=1h4vzfbkxobiq",
	...
	cephalon: {
		...
		websites: {
			official:
				"https://cephalon.cloud/share/register-landing?invite_id=jSdOYA",
...
const NOT_SUPPORT_DEVELOPER_ROLE_PROVIDERS = [
	"poe",
	"qiniu",
] as const satisfies SystemProviderId[];
...
const NOT_SUPPORT_STREAM_OPTIONS_PROVIDERS = [
	"mistral",
] as const satisfies SystemProviderId[];
...
export const isSupportEnableThinkingProvider = (provider: Provider) => {
	return (
		...
		!NOT_SUPPORT_QWEN3_ENABLE_THINKING_PROVIDER.some(
			(pid) => pid === provider.id,
		)
	);
};
...
const SUPPORT_GEMINI_URL_CONTEXT_PROVIDER_TYPES = [
	"gemini",
	"vertexai",
] as const satisfies ProviderType[];

export const isSupportUrlContextProvider = (provider: Provider) => {
	return SUPPORT_GEMINI_URL_CONTEXT_PROVIDER_TYPES.some(
		(type) => type === provider.type,
	);
};

const SUPPORT_GEMINI_NATIVE_WEB_SEARCH_PROVIDERS = [
	"gemini",
	"vertexai",
] as const satisfies SystemProviderId[];
...
export const isGeminiWebSearchProvider = (provider: Provider) => {
	return SUPPORT_GEMINI_NATIVE_WEB_SEARCH_PROVIDERS.some(
		(id) => id === provider.id,
	);
};
...
export const isNewApiProvider = (provider: Provider) => {
	return (
		["new-api", "cherryin"].includes(provider.id) || provider.type === "new-api"
	);
};
