...
export const isTranslateAssistant = (
	assistant: Assistant,
): assistant is TranslateAssistant => {
	return (
		(assistant.model &&
			assistant.targetLanguage &&
			typeof assistant.content === "string") !== undefined
	);
};
...
export type ReasoningEffortOption =
	| NonNullable<OpenAI.ReasoningEffort>
	| "auto";
...
export type ReasoningEffortConfig = Record<
	ThinkingModelType,
 ...
 * @returns 是否为系统内置提供商
 */
export const isSystemProvider = (
	provider: Provider,
): provider is SystemProvider => {
...
export const isAutoDetectionMethod = (
	method: string,
): method is AutoDetectionMethod => {
...
export const isWebSearchProviderId = (
	id: string,
...
export type AISDKWebSearchResult = Omit<
	Extract<LanguageModelV2Source, { sourceType: "url" }>,
	"sourceType"
>;
...
export const isBuiltinMCPServer = (
	server: MCPServer,
): server is BuiltinMCPServer => {
...
export type BuiltinMCPServerName =
	(typeof BuiltinMCPServerNames)[keyof typeof BuiltinMCPServerNames];

export const BuiltinMCPServerNamesArray = Object.values(BuiltinMCPServerNames);

export const isBuiltinMCPServerName = (
	name: string,
): name is BuiltinMCPServerName => {
...
export type MCPToolResponseStatus =
	| "pending"
	| "cancelled"
	| "invoking"
	| "done"
...
export interface MCPToolResponse
	extends Omit<ToolUseResponse | ToolCallResponse, "tool"> {
	tool: MCPTool;
	toolCallId?: string;
	toolUseId?: string;
}
 ...
 */
export function objectEntriesStrict<
	T extends Record<string | number | symbol, unknown>,
>(obj: T): { [K in keyof T]: [K, T[K]] }[keyof T][] {
