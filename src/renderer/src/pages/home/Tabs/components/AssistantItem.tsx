...
const AssistantItem: FC<AssistantItemProps> = ({
	...
	useEffect(() => {
		...
		const hasPending = assistant.topics.some((topic) =>
			hasTopicPendingRequests(topic.id),
		);
	...
	const assistantName = useMemo(
		() => assistant.name || t("chat.default.name"),
		[assistant.name, t],
	);
	...
	return (
		<Dropdown
			...
			trigger={["contextMenu"]}
			popupRender={(menu) => (
				<div onPointerDown={(e) => e.stopPropagation()}>{menu}</div>
			)}
		>
			<Container onClick={handleSwitch} className={isActive ? "active" : ""}>
				...
				{isActive && (
					...
					>
						<TopicCount className="topics-count">
...
const handleTagOperation = (
	...
	const removeTag = () =>
		updateAssistants(
			assistants.map((a) => (a.id === assistant.id ? { ...a, tags: [] } : a)),
		);
	const addTag = () =>
		updateAssistants(
			assistants.map((a) =>
				a.id === assistant.id ? { ...a, tags: [tag] } : a,
			),
		);
...
const createTagMenuItems = (
	...
	const items: MenuProps["items"] = allTags.map((tag) => ({
		label: tag,
		...
		key: `all-tag-${tag}`,
		onClick: () =>
			handleTagOperation(tag, assistant, assistants, updateAssistants),
	}));
	...
	items.push({
		...
		onClick: async () => {
			...
			if (tagName && tagName.trim()) {
				updateAssistants(
					assistants.map((a) =>
						a.id === assistant.id ? { ...a, tags: [tagName.trim()] } : a,
					),
				);
			}
		},
...
function getMenuItems({
	...
	return [
		...
		{
			label: t("assistants.export_web_config"),
			key: "export-web-config",
			icon: <CopyIcon size={14} />,
			onClick: () => {
				const webConfig = {
					assistantId: assistant.id,
					assistantName: assistant.name,
					knowledgeBases: assistant.knowledge_bases || [],
				};
				navigator.clipboard.writeText(JSON.stringify(webConfig, null, 2));
				window.toast.success(t("assistants.web_config_copied"));
			},
		},
		{
		...
		},
		{
			label:
				sortBy === "list"
					? t("assistants.list.showByTags")
					: t("assistants.list.showByList"),
			key: "switch-view",
			icon: sortBy === "list" ? <Tags size={14} /> : <AlignJustify size={14} />,
			onClick: () => {
				sortBy === "list"
					? handleSortByChange?.("tags")
					: handleSortByChange?.("list");
			},
		},
		{
