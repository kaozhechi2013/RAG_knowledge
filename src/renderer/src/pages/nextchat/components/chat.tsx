...
import { ClientApi, type MultimodalContent } from "../client/api";
...
import { type Prompt, usePromptStore } from "../store/prompt";
...
function KnowledgeBaseSelector() {
	...
	const refreshKnowledgeBases = useCallback(() => {
		const apiUrl = accessStore.openaiUrl || "http://localhost:8080";
		const apiKey = accessStore.openaiApiKey || "sk-knowledge-internal-2024";
		console.log("[Knowledge] Fetching from:", apiUrl);
		knowledgeStore.fetchKnowledgeBases(apiUrl, apiKey).catch((error) => {
	...
	useEffect(() => {
		// Auto-fetch knowledge bases on mount - only once
	...
	const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		knowledgeStore.selectKnowledgeBase(value || null);
	};
	...
	return (
		...
		>
			...
			<select
				value={knowledgeStore.selectedKnowledgeBaseId || ""}
			...
			>
				...
				{knowledgeStore.knowledgeBases.map((kb) => (
