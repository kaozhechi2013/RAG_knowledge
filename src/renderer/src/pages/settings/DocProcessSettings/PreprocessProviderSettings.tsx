...
import {
	getPreprocessProviderLogo,
	PREPROCESS_PROVIDER_CONFIG,
} from "@renderer/config/preprocessProviders";
...
const PreprocessProviderSettings: FC<Props> = ({ provider: _provider }) => {
	const { provider: preprocessProvider, updateProvider } =
		usePreprocessProvider(_provider.id);
	...
	const preprocessProviderConfig =
		PREPROCESS_PROVIDER_CONFIG[preprocessProvider.id];
	...
	return (
		<>
			<SettingTitle>
				<Flex align="center" gap={8}>
					...
					{officialWebsite && preprocessProviderConfig?.websites && (
						<Link
							target="_blank"
							href={preprocessProviderConfig.websites.official}
						>
							<ExportOutlined
			...
			{/* 本地OCR不显示API Key配置 */}
			{preprocessProvider.id !== "local-ocr" &&
				hasObjectKey(preprocessProvider, "apiKey") && (
					<>
						...
						>
							...
							<Tooltip
								title={t("settings.provider.api.key.list.open")}
								mouseEnterDelay={0.5}
							>
								<Button
									type="text"
			...
			{preprocessProvider.id === "local-ocr" && (
				<>
					<SettingSubtitle style={{ marginTop: 5, marginBottom: 10 }}>
						{t("settings.preprocess.local_ocr.title", "本地OCR说明")}
					</SettingSubtitle>
					<SettingHelpText>
						{t(
							"settings.preprocess.local_ocr.description",
							"使用本地MinerU进行PDF OCR处理，无需API Key，完全离线可用。首次运行会自动下载OCR模型到本地。",
						)}
					</SettingHelpText>
					<SettingHelpTextRow style={{ marginTop: 10 }}>
						...
						>
							{t("settings.preprocess.local_ocr.docs", "查看MinerU文档")}
						</SettingHelpLink>
					</SettingHelpTextRow>
				</>
			)}
