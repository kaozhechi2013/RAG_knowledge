...
const KnowledgeFiles: FC<KnowledgeContentProps> = ({
	...
	const handleDrop = async (files: File[]) => {
		...
		if (files) {
			...
			if (docFiles.length > 0) {
				const docFileNames = docFiles.map((file) => file.name).join(", ");
				message.error(
					t("knowledge.doc_format_not_supported", { files: docFileNames }),
				);
				// 过滤掉 .doc 文件，继续处理其他文件
	...
	const processFiles = async (files: FileMetadata[]) => {
		...
		if (files.length > 0) {
			// 检查是否有 .doc 格式文件
			const docFiles = files.filter((file) => file.ext === ".doc");
			if (docFiles.length > 0) {
				const docFileNames = docFiles
					.map((file) => file.origin_name || file.name)
					.join(", ");
				message.error(
					t("knowledge.doc_format_not_supported", { files: docFileNames }),
				);
				// 过滤掉 .doc 文件，继续处理其他文件
				files = files.filter((file) => file.ext !== ".doc");
				if (files.length === 0) {
					return;
				}
			}
