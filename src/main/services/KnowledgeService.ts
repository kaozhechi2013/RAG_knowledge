...
import { wordImageCleanerNative } from "@main/knowledge/preprocess/WordImageCleanerNative";
...
class KnowledgeService {
	...
	private pendingDeleteFile = path.join(
		this.storageDir,
		"knowledge_pending_delete.json",
	);
	// Byte based
	private workload = 0;
	private processingItemCount = 0;
	private knowledgeItemProcessingQueueMappingPromise: Map<
		LoaderTaskOfSet,
		() => void
	> = new Map();
	...
	private deleteKnowledgeFile = (id: string): boolean => {
		...
		if (fs.existsSync(dbPath)) {
			try {
				...
				logger.warn(
					`Failed to delete knowledge base file with id: ${id}: ${error}`,
				);
				return false;
			}
		}
		...
		load: (): string[] => {
			try {
				if (fs.existsSync(this.pendingDeleteFile)) {
					return JSON.parse(
						fs.readFileSync(this.pendingDeleteFile, "utf-8"),
					) as string[];
				}
			} catch (error) {
	...
	private cleanupOnStartup = (): void => {
		...
		logger.info(
			`Found ${pendingDeleteIds.length} knowledge bases pending deletion from previous session`,
		);
		...
		pendingDeleteIds.forEach((id) => {
			if (this.deleteKnowledgeFile(id)) {
				...
				logger.warn(
					`Failed to delete knowledge base ${id}, please delete it manually`,
				);
			}
		});
		...
		logger.info(
			`Startup cleanup completed: ${deletedCount}/${pendingDeleteIds.length} knowledge bases deleted`,
		);
	};
	...
	private maximumLoad() {
		return (
			this.processingItemCount >=
				KnowledgeService.MAXIMUM_PROCESSING_ITEM_COUNT ||
			...
			loaderTasks: [
				{
					...
					task: async () => {
						try {
							...
							const fileToProcess: FileMetadata = await this.preprocessing(
								file,
								base,
								item,
								userId,
	...
	private directoryTask(
		...
		const sendDirectoryProcessingPercent = (
			totalFiles: number,
	...
	private processingQueueHandle() {
		const getSubtasksUntilMaximumLoad = (): QueueTaskItem[] => {
			...
			that: for (const [task, resolve] of this
				.knowledgeItemProcessingQueueMappingPromise) {
	...
	private appendProcessingQueue(task: LoaderTask): Promise<LoaderReturn> {
		return new Promise((resolve) => {
			this.knowledgeItemProcessingQueueMappingPromise.set(
				loaderTaskIntoOfSet(task),
				() => {
	...
	public add = (
		_: Electron.IpcMainInvokeEvent,
		options: KnowledgeBaseAddItemOptions,
	...
	public async remove(
		_: Electron.IpcMainInvokeEvent,
		{
			...
			uniqueIds,
			base,
		}: { uniqueId: string; uniqueIds: string[]; base: KnowledgeBaseParams },
	): Promise<void> {
		const ragApplication = await this.getRagApplication(base);
	...
	private preprocessing = async (
		...
		// 1. Word文档图片清理（在所有处理之前）- 使用Native实现
		const isWordDoc = file.ext.toLowerCase() === ".docx";
		if (isWordDoc) {
			try {
				const fileSizeMB = file.size / (1024 * 1024);
				// 检查是否需要清理图片（文件过大）
				if (wordImageCleanerNative.shouldClean(file.path, fileSizeMB)) {
					logger.info(
						`Word文档需要清理图片: ${file.name} (${fileSizeMB.toFixed(2)}MB)`,
					);
					const result = await wordImageCleanerNative.cleanImages(file.path);

					// 更新文件元数据
					fileToProcess = {
						...file,
						size: result.newSize * 1024 * 1024,
					};

					logger.info(
						`Word文档图片清理成功: ${file.name}, 节省${result.savedSize.toFixed(2)}MB (${result.savedPercent.toFixed(1)}%)`,
					);
				} else {
					logger.debug(`Word文档无需清理图片: ${file.name}`);
				}
			} catch (err) {
				logger.error(
					`Word文档图片清理失败，继续使用原文件: ${file.name}`,
					err as Error,
				);
				// 清理失败不应该阻止后续流程，继续使用原文件
				fileToProcess = file;
			}
		}

		// 2. PDF预处理（OCR等）
		if (base.preprocessProvider && fileToProcess.ext.toLowerCase() === ".pdf") {
			try {
				const provider = new PreprocessProvider(
					base.preprocessProvider.provider,
					userId,
				);
				const filePath = fileStorage.getFilePathById(fileToProcess);
				// Check if file has already been preprocessed
				const alreadyProcessed =
					await provider.checkIfAlreadyProcessed(fileToProcess);
				if (alreadyProcessed) {
					logger.debug(
						`File already preprocess processed, using cached result: ${filePath}`,
					);
				...
				logger.debug(
					`Starting preprocess processing for scanned PDF: ${filePath}`,
				);
				const { processedFile, quota } = await provider.parseFile(
					item.id,
					fileToProcess,
				);
	...
	public checkQuota = async (
		...
		try {
			if (
				...
				base.preprocessProvider.type === "preprocess"
			) {
				const provider = new PreprocessProvider(
					base.preprocessProvider.provider,
					userId,
