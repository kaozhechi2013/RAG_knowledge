...
const disableHardwareAcceleration =
	configManager.getDisableHardwareAcceleration();
...
if (!app.requestSingleInstanceLock()) {
	...
	app.whenReady().then(async () => {
		...
		electronApp.setAppUserModelId(
			import.meta.env.VITE_MAIN_BUNDLE_ID || "com.kangfenmao.Knowledge",
		);
		...
		try {
			...
			logger.error("Failed to check/start API server:", error as Error);
		}
	});
	...
	const handleOpenUrl = (args: string[]) => {
		const url = args.find((arg) =>
			arg.startsWith(CHERRY_STUDIO_PROTOCOL + "://"),
		);
