...
const swaggerOptions: swaggerJSDoc.Options = {
	definition: {
		...
		info: {
			...
			version: "1.0.0",
			description:
				"OpenAI-compatible API for Knowledge with additional Knowledge-specific endpoints",
			contact: {
				name: "Cherry Studio",
				...
				ChatCompletionRequest: {
					...
					properties: {
						model: {
							type: "string",
							description:
								"The model to use for completion, in format provider:model-id",
						},
						messages: {
							type: "array",
...
export function setupOpenAPIDocumentation(app: Express) {
	try {
      ...
      `,
				customSiteTitle: "Knowledge API Documentation",
			}),
		);
