...
import {
	openTraceWindow,
	setTraceWindowTitle,
} from "./services/NodeTraceService";
...
export function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
	...
	ipcMain.handle(
		IpcChannel.App_Proxy,
		async (_, proxy: string, bypassRules?: string) => {
	...
	ipcMain.handle(
		IpcChannel.App_SetSpellCheckLanguages,
		(_, languages: string[]) => {
	...
	// launch on boot
	ipcMain.handle(
		IpcChannel.App_SetLaunchOnBoot,
		(_, isLaunchOnBoot: boolean) => {
	...
	ipcMain.handle(
		IpcChannel.App_SetTestChannel,
		async (_, channel: UpgradeChannel) => {
	...
	ipcMain.handle(IpcChannel.App_GetSystemFonts, async () => {
		try {
			...
			return fonts
				.map((font: string) => font.replace(/^"(.*)"$/, "$1"))
	...
	ipcMain.handle(
		...
		(_, key: string, value: any, isNotify: boolean = false) => {
	...
	ipcMain.handle(
		...
		(_, delta: number, reset: boolean = false) => {
			const windows = BrowserWindow.getAllWindows();
	...
	ipcMain.handle(IpcChannel.App_GetCacheSize, async () => {
		...
		try {
			...
			logger.error(
				`Failed to calculate cache size for ${cachePath}: ${error.message}`,
			);
			return "0";
		}
	});
	...
	ipcMain.handle(
		IpcChannel.App_SetStopQuitApp,
		(_, stop: boolean = false, reason: string = "") => {
			if (stop) {
	...
	ipcMain.handle(
		IpcChannel.App_HasWritePermission,
	...
	ipcMain.handle(
		IpcChannel.App_IsPathInside,
		async (_, childPath: string, parentPath: string) => {
	...
	// Relaunch app
	ipcMain.handle(
		IpcChannel.App_RelaunchApp,
		(_, options?: Electron.RelaunchOptions) => {
			// Fix for .AppImage
			if (isLinux && process.env.APPIMAGE) {
				logger.info(
					`Relaunching app with options: ${process.env.APPIMAGE}`,
					options,
				);
	...
	ipcMain.handle(
		...
		async (_, notification: Notification) => {
			await notificationService.sendNotification(notification);
		},
	);
	ipcMain.handle(
		IpcChannel.Notification_OnClick,
	...
	ipcMain.handle(IpcChannel.Zip_Decompress, (_, text: Buffer) =>
		decompress(text),
	);
	...
	ipcMain.handle(
		...
		backupManager.backup.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_Restore,
		backupManager.restore.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_BackupToWebdav,
		backupManager.backupToWebdav.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_RestoreFromWebdav,
		backupManager.restoreFromWebdav.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_ListWebdavFiles,
		backupManager.listWebdavFiles.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_CheckConnection,
		backupManager.checkConnection.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_CreateDirectory,
		backupManager.createDirectory.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_DeleteWebdavFile,
		backupManager.deleteWebdavFile.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_BackupToLocalDir,
		backupManager.backupToLocalDir.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_RestoreFromLocalBackup,
		backupManager.restoreFromLocalBackup.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_ListLocalBackupFiles,
		backupManager.listLocalBackupFiles.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_DeleteLocalBackupFile,
		backupManager.deleteLocalBackupFile.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_BackupToS3,
		backupManager.backupToS3.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_RestoreFromS3,
		backupManager.restoreFromS3.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_ListS3Files,
		backupManager.listS3Files.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_DeleteS3File,
		backupManager.deleteS3File.bind(backupManager),
	);
	ipcMain.handle(
		IpcChannel.Backup_CheckS3Connection,
		backupManager.checkS3Connection.bind(backupManager),
	);
	...
	ipcMain.handle(
		...
		fileManager.openPath.bind(fileManager),
	);
	ipcMain.handle(IpcChannel.File_Save, fileManager.save.bind(fileManager));
	ipcMain.handle(
		IpcChannel.File_Select,
		fileManager.selectFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_Upload,
		fileManager.uploadFile.bind(fileManager),
	);
	...
	ipcMain.handle(
		...
		fileManager.readExternalFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_Delete,
		fileManager.deleteFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_DeleteDir,
		fileManager.deleteDir.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_DeleteExternalFile,
		fileManager.deleteExternalFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_DeleteExternalDir,
		fileManager.deleteExternalDir.bind(fileManager),
	);
	ipcMain.handle(IpcChannel.File_Move, fileManager.moveFile.bind(fileManager));
	ipcMain.handle(
		IpcChannel.File_MoveDir,
		fileManager.moveDir.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_Rename,
		fileManager.renameFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_RenameDir,
		fileManager.renameDir.bind(fileManager),
	);
	ipcMain.handle(IpcChannel.File_Get, fileManager.getFile.bind(fileManager));
	ipcMain.handle(
		IpcChannel.File_SelectFolder,
		fileManager.selectFolder.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_CreateTempFile,
		fileManager.createTempFile.bind(fileManager),
	);
	ipcMain.handle(IpcChannel.File_Mkdir, fileManager.mkdir.bind(fileManager));
	ipcMain.handle(
		IpcChannel.File_Write,
		fileManager.writeFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_WriteWithId,
		fileManager.writeFileWithId.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_SaveImage,
		fileManager.saveImage.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_Base64Image,
		fileManager.base64Image.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_SaveBase64Image,
		fileManager.saveBase64Image.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_SavePastedImage,
		fileManager.savePastedImage.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_Base64File,
		fileManager.base64File.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_GetPdfInfo,
		fileManager.pdfPageCount.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_Download,
		fileManager.downloadFile.bind(fileManager),
	);
	ipcMain.handle(IpcChannel.File_Copy, fileManager.copyFile.bind(fileManager));
	ipcMain.handle(
		IpcChannel.File_BinaryImage,
		fileManager.binaryImage.bind(fileManager),
	);
	ipcMain.handle(
	...
	ipcMain.handle(
		...
		fileManager.isTextFile.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_GetDirectoryStructure,
		fileManager.getDirectoryStructure.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_CheckFileName,
		fileManager.fileNameGuard.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_ValidateNotesDirectory,
		fileManager.validateNotesDirectory.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_StartWatcher,
		fileManager.startFileWatcher.bind(fileManager),
	);
	ipcMain.handle(
		IpcChannel.File_StopWatcher,
		fileManager.stopFileWatcher.bind(fileManager),
	);

	// file service
	ipcMain.handle(
		IpcChannel.FileService_Upload,
		async (_, provider: Provider, file: FileMetadata) => {
	...
	ipcMain.handle(
		IpcChannel.FileService_Delete,
		async (_, provider: Provider, fileId: string) => {
			const service = FileServiceManager.getInstance().getService(provider);
	...
	ipcMain.handle(
		IpcChannel.FileService_Retrieve,
		async (_, provider: Provider, fileId: string) => {
			const service = FileServiceManager.getInstance().getService(provider);
	...
	ipcMain.handle(
		...
		FileService.readTextFileWithAutoEncoding.bind(FileService),
	);

	// export
	ipcMain.handle(
		IpcChannel.Export_Word,
		exportService.exportToWord.bind(exportService),
	);
	...
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Create,
		KnowledgeService.create.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Reset,
		KnowledgeService.reset.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Delete,
		KnowledgeService.delete.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Add,
		KnowledgeService.add.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Remove,
		KnowledgeService.remove.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Search,
		KnowledgeService.search.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Rerank,
		KnowledgeService.rerank.bind(KnowledgeService),
	);
	ipcMain.handle(
		IpcChannel.KnowledgeBase_Check_Quota,
		KnowledgeService.checkQuota.bind(KnowledgeService),
	);

	// Get assistant configuration from renderer process
	ipcMain.handle(
		IpcChannel.KnowledgeBase_GetAssistantConfig,
		async (_, assistantId: string) => {
			try {
				// Send request to renderer process through main window
				if (!mainWindow) {
					logger.error("Main window not available");
					return null;
				}

				// Request assistant config from renderer
				mainWindow.webContents.send(
					"get-assistant-config-request",
					assistantId,
				);
				...
				return new Promise((resolve) => {
					const timeout = setTimeout(() => {
						logger.warn(`Timeout waiting for assistant config: ${assistantId}`);
						resolve(null);
					}, 5000);

					ipcMain.once(
						`get-assistant-config-response-${assistantId}`,
						(_, config) => {
				...
				logger.error("Failed to get assistant config:", error as Error);
				return null;
			}
		},
	);
	...
	ipcMain.handle(
		IpcChannel.Memory_DeleteAllMemoriesForUser,
		async (_, userId) => {
	...
	// window
	ipcMain.handle(
		IpcChannel.Windows_SetMinimumSize,
	...
	ipcMain.handle(
		...
		async (_, projectId: string, clientEmail?: string) => {
	...
	ipcMain.handle(IpcChannel.MiniWindow_SetPin, (_, isPinned) =>
		windowService.setPinMiniWindow(isPinned),
	);

	// aes
	ipcMain.handle(
		IpcChannel.Aes_Encrypt,
		(_, text: string, secretKey: string, iv: string) =>
			encrypt(text, secretKey, iv),
	);
	ipcMain.handle(
		IpcChannel.Aes_Decrypt,
		(_, encryptedData: string, iv: string, secretKey: string) =>
	...
	ipcMain.handle(
		IpcChannel.Mcp_CheckConnectivity,
		mcpService.checkMcpConnectivity,
	);
	...
	ipcMain.handle(IpcChannel.App_IsBinaryExist, (_, name: string) =>
		isBinaryExists(name),
	);
	ipcMain.handle(IpcChannel.App_GetBinaryPath, (_, name: string) =>
		getBinaryPath(name),
	);
	ipcMain.handle(IpcChannel.App_InstallUvBinary, () =>
		runInstallScript("install-uv.js"),
	);
	ipcMain.handle(IpcChannel.App_InstallBunBinary, () =>
		runInstallScript("install-bun.js"),
	);
	ipcMain.handle(IpcChannel.App_InstallOvmsBinary, () =>
		runInstallScript("install-ovms.js"),
	);

	//copilot
	ipcMain.handle(
		IpcChannel.Copilot_GetAuthMessage,
		CopilotService.getAuthMessage.bind(CopilotService),
	);
	ipcMain.handle(
		IpcChannel.Copilot_GetCopilotToken,
		CopilotService.getCopilotToken.bind(CopilotService),
	);
	ipcMain.handle(
		IpcChannel.Copilot_SaveCopilotToken,
		CopilotService.saveCopilotToken.bind(CopilotService),
	);
	ipcMain.handle(
		IpcChannel.Copilot_GetToken,
		CopilotService.getToken.bind(CopilotService),
	);
	ipcMain.handle(
		IpcChannel.Copilot_Logout,
		CopilotService.logout.bind(CopilotService),
	);
	ipcMain.handle(
		IpcChannel.Copilot_GetUser,
		CopilotService.getUser.bind(CopilotService),
	);
	...
	// nutstore
	ipcMain.handle(
		IpcChannel.Nutstore_GetSsoUrl,
		NutstoreService.getNutstoreSSOUrl.bind(NutstoreService),
	);
	ipcMain.handle(IpcChannel.Nutstore_DecryptToken, (_, token: string) =>
		NutstoreService.decryptToken(token),
	);
	ipcMain.handle(
		IpcChannel.Nutstore_GetDirectoryContents,
		(_, token: string, path: string) =>
	...
	ipcMain.handle(
		IpcChannel.SearchWindow_OpenUrl,
		async (_, uid: string, url: string) => {
	...
	// webview
	ipcMain.handle(
		IpcChannel.Webview_SetOpenLinkExternal,
		(_, webviewId: number, isExternal: boolean) =>
			setOpenLinkExternal(webviewId, isExternal),
	);
	ipcMain.handle(
		IpcChannel.Webview_SetSpellCheckEnabled,
		(_, webviewId: number, isEnable: boolean) => {
	...
	ipcMain.handle(IpcChannel.TRACE_SAVE_DATA, (_, topicId: string) =>
		saveSpans(topicId),
	);
	ipcMain.handle(
		IpcChannel.TRACE_GET_DATA,
		(_, topicId: string, traceId: string, modelName?: string) =>
			getSpans(topicId, traceId, modelName),
	);
	ipcMain.handle(IpcChannel.TRACE_SAVE_ENTITY, (_, entity: SpanEntity) =>
		saveEntity(entity),
	);
	ipcMain.handle(IpcChannel.TRACE_GET_ENTITY, (_, spanId: string) =>
		getEntity(spanId),
	);
	ipcMain.handle(
		IpcChannel.TRACE_BIND_TOPIC,
		(_, topicId: string, traceId: string) => bindTopic(traceId, topicId),
	);
	ipcMain.handle(
		IpcChannel.TRACE_CLEAN_TOPIC,
		(_, topicId: string, traceId?: string) => cleanTopic(topicId, traceId),
	);
	ipcMain.handle(
		IpcChannel.TRACE_TOKEN_USAGE,
		(_, spanId: string, usage: TokenUsage) => tokenUsage(spanId, usage),
	);
	ipcMain.handle(
		IpcChannel.TRACE_CLEAN_HISTORY,
		(_, topicId: string, traceId: string, modelName?: string) =>
	...
	ipcMain.handle(IpcChannel.TRACE_SET_TITLE, (_, title: string) =>
		setTraceWindowTitle(title),
	);
	ipcMain.handle(
		IpcChannel.TRACE_ADD_END_MESSAGE,
		(_, spanId: string, modelName: string, message: string) =>
	...
	ipcMain.handle(
		IpcChannel.Anthropic_CompleteOAuthWithCode,
		(_, code: string) => anthropicService.completeOAuthWithCode(code),
	);
	...
	ipcMain.handle(IpcChannel.CodeTools_GetAvailableTerminals, () =>
		codeToolsService.getAvailableTerminalsForPlatform(),
	);
	ipcMain.handle(
		IpcChannel.CodeTools_SetCustomTerminalPath,
		(_, terminalId: string, path: string) =>
			codeToolsService.setCustomTerminalPath(terminalId, path),
	);
	ipcMain.handle(
		IpcChannel.CodeTools_GetCustomTerminalPath,
		(_, terminalId: string) =>
			codeToolsService.getCustomTerminalPath(terminalId),
	);
	ipcMain.handle(
		IpcChannel.CodeTools_RemoveCustomTerminalPath,
		(_, terminalId: string) =>
	...
	ipcMain.handle(
		IpcChannel.OCR_ocr,
		(_, file: SupportedOcrFile, provider: OcrProvider) =>
			ocrService.ocr(file, provider),
	);
	ipcMain.handle(IpcChannel.OCR_ListProviders, () =>
		ocrService.listProviderIds(),
	);
	...
	ipcMain.handle(IpcChannel.Ovms_StopAddModel, () =>
		ovmsManager.stopAddModel(),
	);
