...
export default class Doc2xPreprocessProvider extends BasePreprocessProvider {
	...
	private async validateFile(filePath: string): Promise<void> {
		...
		if (fileSizeBytes >= 300 * 1024 * 1024) {
			...
			throw new Error(
				`PDF file size (${fileSizeMB}MB) exceeds the limit of 300MB`,
			);
		}
		...
		if (doc.numPages >= 1000) {
			throw new Error(
				`PDF page count (${doc.numPages}) exceeds the limit of 1000 pages`,
			);
		}
	}
	...
	public async parseFile(
		sourceId: string,
		file: FileMetadata,
	): Promise<{ processedFile: FileMetadata }> {
		try {
	...
	private async waitForProcessing(
		...
		while (true) {
			...
			logger.info(
				`Preprocess processing status: ${status}, progress: ${progress}%`,
			);
		...
		try {
			...
			if (!response.ok) {
				if (response.status === 401) {
					throw new Error(
						`认证失败: API Key无效或未设置。请在知识库设置中配置正确的Doc2x API Key，或禁用PDF预处理功能。`,
					);
				}
			...
			if (data.code === "success" && data.data) {
				...
				throw new Error(
					`API returned error: ${data.message || JSON.stringify(data)}`,
				);
			}
		} catch (error) {
			logger.error(
				`Failed to get preupload URL: ${error instanceof Error ? error.message : String(error)}`,
			);
	...
	private async getStatus(uid: string): Promise<StatusResponse> {
		...
		try {
			...
			logger.error(
				`Failed to get status for uid ${uid}: ${error instanceof Error ? error.message : String(error)}`,
			);
	...
	private async convertFile(uid: string, filePath: string): Promise<void> {
		...
		try {
			...
			logger.error(
				`Failed to convert file ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
			);
	...
	private async downloadFile(
		...
		try {
			...
			logger.error(
				`Failed to download and extract file: ${error instanceof Error ? error.message : String(error)}`,
			);
