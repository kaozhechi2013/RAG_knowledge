...
export default class MineruPreprocessProvider extends BasePreprocessProvider {
	constructor(provider: PreprocessProvider, userId?: string) {
		...
		this.provider.apiKey =
			this.provider.apiKey || import.meta.env.MAIN_VITE_MINERU_API_KEY;
	}
	...
	public async parseFile(
		...
		try {
			...
			// 2. 等待处理完成并获取结果
			const extractResult = await this.waitForCompletion(
				sourceId,
				batchId,
				file.origin_name,
			);
			...
			// 3. 下载并解压文件
			const { path: outputPath } = await this.downloadAndExtractFile(
	...
	public async checkQuota() {
		try {
			...
			if (!quota.ok) {
				if (quota.status === 401) {
					throw new Error(
						`认证失败: API Key无效或未设置。请在知识库设置中配置正确的MinerU API Key，或禁用PDF预处理功能。`,
					);
				}
	...
	private async validateFile(filePath: string): Promise<void> {
		...
		if (doc.numPages >= 600) {
			throw new Error(
				`PDF page count (${doc.numPages}) exceeds the limit of 600 pages`,
			);
		}
		// 文件大小小于200MB
		if (pdfBuffer.length >= 200 * 1024 * 1024) {
			...
			throw new Error(
				`PDF file size (${fileSizeMB}MB) exceeds the limit of 200MB`,
			);
		}
	}
		...
		try {
			...
			if (mdFile) {
				...
				try {
					...
					logger.warn(
						`Failed to rename file ${mdFile} to ${finalName}: ${renameError}`,
					);
					// 如果重命名失败，使用原文件
	...
	private async downloadAndExtractFile(
		zipUrl: string,
		file: FileMetadata,
	): Promise<{ path: string }> {
		...
		try {
			...
			if (response.ok) {
				...
				if (data.code === 0 && data.data) {
					...
					throw new Error(
						`API returned error: ${data.msg || JSON.stringify(data)}`,
					);
				}
			} else {
				throw new Error(`HTTP ${response.status}: ${response.statusText}`);
		...
		try {
			...
			if (!response.ok) {
				...
				try {
					...
					throw new Error(
						`Upload failed with status ${response.status}: ${responseBody}`,
					);
				} catch (parseError) {
					throw new Error(
						`Upload failed with status ${response.status}. Could not parse response body.`,
					);
				}
			}
	...
	private async getExtractResults(
		batchId: string,
		...
		try {
			...
			logger.error(
				`Failed to get extract results for batch ${batchId}: ${error.message}`,
			);
			throw new Error(error.message);
		}
	}
	...
	private async waitForCompletion(
		...
		while (retries < maxRetries) {
			try {
				...
				// 查找对应文件的处理结果
				const fileResult = result.extract_result.find(
				...
				if (fileResult.state === "done" && fileResult.full_zip_url) {
					...
					throw new Error(
						`Processing failed for file: ${fileName}, error: ${fileResult.err_msg}`,
					);
				} else if (fileResult.state === "running") {
					// 发送进度更新
					if (fileResult.extract_progress) {
						const progress = Math.round(
							(fileResult.extract_progress.extracted_pages /
								fileResult.extract_progress.total_pages) *
								100,
						);
				...
				logger.warn(
					`Failed to check status for batch ${batchId}, retry ${retries + 1}/${maxRetries}`,
				);
